...
def main():
    ...
    for case_dir in in_dir.iterdir():
        ...
        for vessel_name in vessels:
            vessel_path = segmentation_dir / vessel_name

            if vessel_path.exists():
                print(f"Fodun vessel: {vessel_path}")
                files.append(vessel_path)
    ...
    for i, f in enumerate(files, 1):
        ...
        vessel_name = f.name.replace(".nii.gz", "").replace(".nii", "")
        out_path = out_dir / case_id / f"{vessel_name}_centerline.nii.gz"
        # keep filename; just write NIfTI (same extension)
        try:
            process_one(f, out_path, args.threshold, args.keep_lcc)
            ok += 1
            print(f"[{i}/{len(files)}] OK: {case_id}/{vessel_name}")
        except Exception as e:
            fail += 1
            print(f"[{i}/{len(files)}] FAIL: {case_id}/{vessel_name} -> {e}", file=sys.stderr)
